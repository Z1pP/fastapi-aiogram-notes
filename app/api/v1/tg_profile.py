...
async def get_all_tg_profiles(session: AsyncSession = Depends(get_async_session)):
...
async def create_tg_profile(
    new_profile: TgProfileCreate, session: AsyncSession = Depends(get_async_session)
):
    tg_profile_service = TgProfileService(session)
    try:
...
async def get_tg_profile_by_tg_id(
    tg_id: int, session: AsyncSession = Depends(get_async_session)
):
    tg_profile_service = TgProfileService(session)
    try:
...
async def update_tg_profile(
    update_data: TgProfileUpdate, session: AsyncSession = Depends(get_async_session)
):
    tg_profile_service = TgProfileService(session)
    try:
